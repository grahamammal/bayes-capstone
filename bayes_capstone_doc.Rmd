...
### cite https://towardsdatascience.com/introduction-to-aic-akaike-information-criterion-9c9ba1c96ced

Given small sample sizes, it is more likely to use AICc, essentially AIC with an additional penality term:

$$AICc_M = AIC_M + \frac{2k^2 + 2k}{n - k -1}$$

## then we can remove this eqn below

\[
p_k = \frac{exp\left(-\frac{1}{2}(AIC_{min} - AIC_k)\right)}{\sum_{i=1}^K exp\left(-\frac{1}{2}(AIC_{min} - AIC_k)\right)}
\]

AIC fits well in the freqentist frame. The likelihood component is assumed to come from the maximum likelihood estimates of paramters, which Bayesian methods do not select for, and point estimates for parameters are used, instead of using the full posterior distribution.
...
=======
BIC penalizes free parameters more than the AIC when data sets are large. While AIC tries to select the best model that describes the data presented, the BIC attempts to select the *true* model from among a model set. However, like AIC, BIC does not naturally fit with the Bayesian framework as it is scored by the likelihood function. 


### cite this with https://www.statisticshowto.com/bayesian-information-criterion/
>>>>>>> 9f08d42d8228b93998b627b1147e428a6782d0d3
...
Once the models are calculated, we calcuate Leave One Out expected log point predictive densities (ELPD LOO) for each model. These are shown below. 
...
```{r}
formattable(lpd, align=c("l"), col.names=c("Model 1", "Model 2", "Model 3", "Model 4"))
